...
from app import app, db, newsapi
...
@app.route('/home_Cap_News', methods=['GET'])
@login_required
def home_Cap_News():
    title = {'Capstone News| HOME'}
...
def home():
    form = PostForm()
    #news =head_lines
    return render_template('getnews.html', form=form, news='')
...
def getnews():
    form = PostForm()
    keyword = request.form.get('keyword')  #getting input from user
    news = newsapi.get_top_headlines(q=keyword,
                                     ...
                                     country='us')
    #print(news['articles'])
    return render_template('getnews.html', form=form, news=news['articles'])
